...
df = pd.read_excel(fp,sheet_name='LCOM Plots (5)',header=1,index_col=1)
...
for idx, item in enumerate(total):
    ...
    if idx < 3:
        ...
        xlabel = plt.text(idx+.5,-.12,df.columns.values[idx][:-2],fontsize=12,
                      horizontalalignment='center',verticalalignment='top')
...
L.set_alpha(0)
# plt.show()
plt.savefig('Bar_chart_mpl_CO_H2.png',dpi=300)